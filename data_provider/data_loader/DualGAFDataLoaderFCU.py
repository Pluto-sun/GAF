...
class DualGAFDataManager:
    ...
    def __init__(self, args):
        ...
        #     "DualDuct_Fouling_Cooling_Airside_Minor_direct_5min_working.csv": "Fouling_Cooling_Airside_Minor",
        #     "DualDuct_Fouling_Cooling_Airside_Severe_direct_5min_working.csv": "Fouling_Cooling_Airside_Severe",
        #     "DualDuct_Fouling_Cooling_Waterside_Minor_direct_5min_working.csv": "Fouling_Cooling_Waterside_Minor",
        #     "DualDuct_Fouling_Cooling_Waterside_Severe_direct_5min_working.csv": "Fouling_Cooling_Waterside_Severe",
        #     "DualDuct_Fouling_Heating_Airside_Minor_direct_5min_working.csv": "Fouling_Heating_Airside_Minor",
        #     "DualDuct_Fouling_Heating_Airside_Severe_direct_5min_working.csv": "Fouling_Heating_Airside_Severe",
        #     "DualDuct_Fouling_Heating_Waterside_Minor_direct_5min_working.csv": "Fouling_Heating_Waterside_Minor",
        ...
        # }
        self.file_label_map = {
            "FCU_Control_CoolingReverse_direct_5min_working.csv": "CoolingReverse",
            "FCU_Control_HeatingReverse_direct_5min_working.csv": "HeatingReverse",
            "FCU_Control_Unstable_direct_5min_working.csv": "Unstable",
            "FCU_FanOutletBlockage_direct_5min_working.csv": "FanOutletBlockage",
            "FCU_FaultFree_direct_5min_working.csv": "FaultFree",
            "FCU_FilterRestriction_10%_direct_5min_working.csv": "FilterRestriction_10%",
            "FCU_FilterRestriction_50%_direct_5min_working.csv": "FilterRestriction_50%",
            "FCU_Fouling_Cooling_Airside_Minor_direct_5min_working.csv": "Fouling_Cooling_Airside_Minor",
            "FCU_Fouling_Cooling_Airside_Severe_direct_5min_working.csv": "Fouling_Cooling_Airside_Severe",
            "FCU_Fouling_Cooling_Waterside_Minor_direct_5min_working.csv": "Fouling_Cooling_Waterside_Minor",
            "FCU_Fouling_Cooling_Waterside_Severe_direct_5min_working.csv": "Fouling_Cooling_Waterside_Severe",
            "FCU_Fouling_Heating_Airside_Minor_direct_5min_working.csv": "Fouling_Heating_Airside_Minor",
            "FCU_Fouling_Heating_Airside_Severe_direct_5min_working.csv": "Fouling_Heating_Airside_Severe",
            "FCU_Fouling_Heating_Waterside_Minor_direct_5min_working.csv": "Fouling_Heating_Waterside_Minor",
            "FCU_Fouling_Heating_Waterside_Severe_direct_5min_working.csv": "Fouling_Heating_Waterside_Severe"
        }
    ...
    def _init_parallel_config(self):
        ...
        self.n_jobs = getattr(
            self.args, "n_jobs", min(mp.cpu_count(), 8)
        )  # 限制最大进程数
        self.use_multiprocessing = getattr(self.args, "use_multiprocessing", True)
        self.chunk_size = getattr(
            self.args, "chunk_size", 100
        )  # 每个进程处理的数据块大小
        self.use_shared_memory = getattr(
            self.args, "use_shared_memory", True
        )  # 启用共享内存优化

        print(
            f"🚀 初始化并行处理配置 - 进程数: {self.n_jobs}, 使用多进程: {self.use_multiprocessing}, 共享内存: {self.use_shared_memory}"
        )
    ...
    def _process_data(self):
        ...
        gaf_summation_data = self.generate_gaf_matrix_shared_memory(
            labeled_windows, "summation", False
        )
        ...
        gaf_difference_data = self.generate_gaf_matrix_shared_memory(
            labeled_windows, "difference", False
        )
        print(f"Difference GAF转换后数据形状: {gaf_difference_data.shape}")
        difference_time = time.time() - start_time

        print(
            f"GAF转换耗时 - Summation: {summation_time:.2f}s, Difference: {difference_time:.2f}s"
        )

        # 数据类型转换
        print("\n=== 开始数据范围转换（Summation GAF） ===")
        start_time = time.time()
        gaf_summation_data = self.convert_gaf_data_type_shared_memory(
            gaf_summation_data
        )
        ...
        start_time = time.time()
        gaf_difference_data = self.convert_gaf_data_type_shared_memory(
            gaf_difference_data
        )
        difference_convert_time = time.time() - start_time

        print(
            f"数据类型转换耗时 - Summation: {summation_convert_time:.2f}s, Difference: {difference_convert_time:.2f}s"
        )

        print(
            f"Summation GAF 数据范围: [{gaf_summation_data.min()}, {gaf_summation_data.max()}]"
        )
        print(
    ...
    def generate_gaf_matrix_shared_memory(
        self, data: np.ndarray, method: str = "summation", normalize: bool = False
    ) -> np.ndarray:
        ...
        print(
            f"输入大小: {data_size_gb:.2f}GB, 预估输出大小: {estimated_output_size_gb:.2f}GB"
        )
        ...
        if (
            N < min_samples_for_shared_memory
            or not self.use_multiprocessing
            or not self.use_shared_memory
            or estimated_output_size_gb > max_memory_gb
        ):

            print(
                f"使用标准并行处理 - 数据量: {N}, 输出大小: {estimated_output_size_gb:.2f}GB"
            )
        ...
        try:
            ...
            result_array = np.ndarray(
                result_shape, dtype=result_dtype, buffer=result_shm.buf
            )
            ...
            print(
                f"  原始块大小: {self.chunk_size} -> 优化块大小: {optimal_chunk_size}"
            )
            print(
                f"  实际块数: {len(chunk_indices)}, 数据大小: {estimated_output_size_gb:.1f}GB"
            )
                ...
                for future in as_completed(futures):
                    try:
                        ...
                        if completed % max(1, len(futures) // 5) == 0:
                            print(
                                f"共享内存GAF处理进度: {completed}/{len(futures)} ({100*completed/len(futures):.1f}%)"
                            )
    ...
    def generate_gaf_matrix_parallel(
        self, data: np.ndarray, method: str = "summation", normalize: bool = False
    ) -> np.ndarray:
        """并行GAF矩阵生成函数"""
        if data.ndim != 3:
        ...
        if N < min_samples_for_multiprocess or not self.use_multiprocessing:
            print(
                f"数据量较小({N} < {min_samples_for_multiprocess})或禁用多进程，使用单进程处理"
            )
            return self._generate_gaf_matrix_single(data, method, normalize)
        ...
        print(
            f"  原始进程数: {self.n_jobs} -> 有效进程数: {effective_n_jobs} -> 最终进程数: {final_n_jobs}"
        )
    ...
    def _generate_gaf_matrix_single(
        self, data: np.ndarray, method: str = "summation", normalize: bool = False
    ) -> np.ndarray:
        """单进程GAF矩阵生成函数"""
        N, T, D = data.shape
    ...
    @staticmethod
    def _process_gaf_chunk(
        chunk_data: np.ndarray, method: str, normalize: bool
    ) -> np.ndarray:
    ...
    @staticmethod
    def _process_gaf_chunk_shared_memory(
        shm_name: str,
        shape: tuple,
        dtype,
        start_idx: int,
    ...
    def convert_gaf_data_type_parallel(self, data: np.ndarray) -> np.ndarray:
        """并行GAF数据类型转换"""
        print(
            f"开始并行数据类型转换，数据形状: {data.shape}, 内存占用: {data.nbytes / 1024**3:.2f} GB"
        )
    ...
    @staticmethod
    def _process_conversion_chunk_shared_memory(
        input_shm_name: str,
        input_shape: tuple,
        input_dtype,
        result_shm_name: str,
        ...
        try:
            ...
            input_array = np.ndarray(
                input_shape, dtype=input_dtype, buffer=input_shm.buf
            )
    ...
    def convert_gaf_data_type_shared_memory(self, data: np.ndarray) -> np.ndarray:
        """使用共享内存的并行GAF数据类型转换"""
        print(
            f"开始共享内存数据类型转换，数据形状: {data.shape}, 内存占用: {data.nbytes / 1024**3:.2f} GB"
        )
        ...
        try:
            ...
            result_shm = shared_memory.SharedMemory(create=True, size=result_size)
            result_array = np.ndarray(
                data.shape, dtype=target_dtype, buffer=result_shm.buf
            )
            ...
            print(
                f"使用{optimal_workers}个线程处理{len(chunk_indices)}个块，块大小约{optimal_chunk_size}"
            )
                ...
                for future in as_completed(futures):
                    try:
                        ...
                        if completed % max(1, len(futures) // 3) == 0:
                            print(
                                f"共享内存转换进度: {completed}/{len(futures)} ({100*completed/len(futures):.1f}%)"
                            )
                    except Exception as exc:
            ...
            print(
                f"共享内存转换完成，输出类型: {final_result.dtype}, 大小: {final_result.nbytes / 1024**3:.2f}GB"
            )
            return final_result
    ...
    def normalize_features_parallel(
        self, all_segments: list, feature_columns: list
    ) -> None:
        """并行特征归一化处理"""
        print("\n=== 开始并行特征归一化 ===")
        start_time = time.time()
        ...
        if self.use_multiprocessing and len(feature_columns) > 1:
            ...
            with ThreadPoolExecutor(
                max_workers=min(self.n_jobs, len(feature_columns))
            ) as executor:
                future_to_col = {}
                ...
                for future in as_completed(future_to_col):
                    ...
                    try:
                        ...
                        if i % 5 == 0:
                            print(
                                f"完成特征 {i+1}/{len(feature_columns)}: {col} 的scaler训练"
                            )
                    except Exception as exc:
            ...
            with ThreadPoolExecutor(max_workers=self.n_jobs) as executor:
                # 分块处理数据段
                segment_chunks = self._split_segments_into_chunks(
                    all_segments, 50
                )  # 每个线程处理50个数据段
                ...
                for chunk_idx, segment_chunk in enumerate(segment_chunks):
                    future = executor.submit(
                        self._apply_normalization_to_chunk,
                        segment_chunk,
                        feature_columns,
                    )
                ...
                for future in as_completed(future_to_idx):
                    ...
                    try:
                        ...
                        print(
                            f"完成第 {chunk_idx+1}/{len(segment_chunks)} 批数据段的归一化"
                        )
                    except Exception as exc:
    ...
    def _apply_normalization_to_chunk(
        self, segment_chunk: list, feature_columns: list
    ) -> list:
        """对一批数据段应用归一化"""
        normalized_chunk = []
    ...
    def _process_batch_int(
        self, batch_data: np.ndarray, dtype, max_val: int
    ) -> np.ndarray:
        """整数批处理"""
        data_min, data_max = batch_data.min(), batch_data.max()
        if data_min >= -1.0 and data_max <= 1.0:
    ...
    def load_persisted_data(self, path):
        ...
        if "data_type_method" in data:
            ...
            if saved_method != self.data_type_method:
                print(
                    f"警告：持久化文件的数据类型方法({saved_method})与当前设置({self.data_type_method})不匹配"
                )
...
class DualGAFDataLoaderFCU(Dataset):
    """
    双GAF数据集视图
