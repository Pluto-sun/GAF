...
# echo "=== 实验1: 基础膨胀卷积配置 (dilated + adaptive + channel + mlp) ==="
# python run.py \
#     --task_name $task_name \
#     --is_training 1 \
#     --model_id $model_id \
...
#     --classifier_type mlp \
#     --des "dilated-adaptive-channel-mlp"

# # 实验2: 大核特征提取器 + 拼接融合 + 空间注意力
# echo "=== 实验2: 膨胀卷积特征提取器配置 (dilated + concat + spatial + mlp) ==="
# python run.py \
#     --task_name $task_name \
#     --is_training 1 \
#     --model_id $model_id \
...
#     --classifier_type mlp \
#     --des "dilated-concat-spatial-mlp"

# # 实验3: 大卷积核特征提取器 + 加权相加融合 + CBAM注意力
# echo "=== 实验3: 膨胀卷积配置 (dilated + weighted_add + cbam + simple) ==="
# python run.py \
#     --task_name $task_name \
#     --is_training 1 \
#     --model_id $model_id \
...
#     --attention_type cbam \
#     --classifier_type mlp \
#     --des "dilated-weighted-cbam-mlp"

# # 实验4: 大卷积核特征提取器 + 元素相乘融合 + 自注意力
# echo "=== 实验4: 膨胀卷积配置 (dilated + mul + self + mlp) ==="
# python run.py \
#     --task_name $task_name \
#     --is_training 1 \
#     --model_id $model_id \
...
#     --attention_type self \
#     --classifier_type mlp \
#     --des "dilated-mul-self-mlp"

# # 实验5: 大卷积核特征提取器 + 简单相加融合 + 无注意力机制
# echo "=== 实验5: 膨胀卷积配置 (dilated + add + none + simple) "
# python run.py \
#     --task_name $task_name \
#     --is_training 1 \
#     --model_id $model_id \
...
#     --attention_type none \
#     --classifier_type mlp \
#     --des "dilated-add-none-mlp"

# # 实验6: 最优配置组合测试 (更大的特征维度)
# echo "=== 实验6: 膨胀卷积高维特征配置 (dilated + adaptive + channel + mlp, feature_dim=128) ==="
# python run.py \
#     --task_name $task_name \
#     --is_training 1 \
#     --model_id $model_id \
...
#     --attention_type cbam \
#     --classifier_type mlp \
#     --des "dilated-high-dim-128"
    
# # 实验7: 膨胀卷积配置 (dilated + bidirectional + cbam + mlp)
# echo "=== 实验7: 膨胀卷积配置 (dilated + bidirectional + cbam + mlp) ==="
# python run.py \
#     --task_name $task_name \
#     --is_training 1 \
#     --model_id $model_id \
...
#     --attention_type cbam \
#     --classifier_type mlp \
#     --des "dilated-bidirectional-cbam-mlp"

# 实验8: 膨胀卷积配置 (dilated + gated + cbam + mlp)
echo "=== 实验8: 膨胀卷积配置 (dilated + gated + cbam + mlp) ==="
python run.py \
    --task_name $task_name \
    --is_training 1 \
    --model_id $model_id \
    ...
    --attention_type cbam \
    --classifier_type mlp \
    --des "dilated-gated-cbam-mlp"

# 实验9: 膨胀卷积配置 (dilated + bidirectional + cbam + mlp)
echo "=== 实验9: 膨胀卷积配置 (dilated + bidirectional + cbam + mlp) ==="
python run.py \
    --task_name $task_name \
    --is_training 1 \
    --model_id $model_id \
    ...
    --attention_type self \
    --classifier_type mlp \
    --des "dilated-bidirectional-self-mlp"

# 实验10: 膨胀卷积配置 (dilated + bidirectional + cbam + mlp)
echo "=== 实验10: 膨胀卷积大维度配置 (dilated + bidirectional + cbam + mlp) ==="
python run.py \
    --task_name $task_name \
    --is_training 1 \
    --model_id $model_id \
    ...
    --attention_type cbam \
    --classifier_type mlp \
    --des "dilated-bidirectional-cbam-mlp-large-dim"
